...
def appendFile(fname, dl=None, validate=True, person_id='docID', input_param_file=0, token=accessToken):
    ...
    if input_param_file:
        ...
        for param in configDict["touchPointColumns"].keys():  # remove the touch point params that aren't populated
            try:
                configDict["touchPointColumns"][param] = int(configDict["touchPointColumns"][param])
            except ValueError:
                configDict["touchPointColumns"].pop(param)
        for param in configDict["passThroughColumns"].keys():  # remove the pass through params that aren't populated
            try:
        ...
        if str(configDict["fileFormat"]["header"]).lower() == 'y': head_row = of.next()  # handle header row
...
def createNewFile(fname, dl, person_id):
    ...
    if person_id == 'clink':
        nf.write('ak' + dl + dl.join(headers.keys()) + dl + 'clink')
    else:
        nf.write('ak' + dl + dl.join(headers.keys()) + dl + 'docID')
...
def createFailedFile(fname, dl):
    ...
    headers.update(configDict["touchPointColumns"])
    headers.update(configDict["passThroughColumns"])
    ff.write('ak' + dl + dl.join(headers.keys()) + dl + 'status' + '\n')    # write header row in failed file
    ff.close()
    return failed_fname
...
def endpointOptionQuery():
    ...
    for key in configDict["endpointOptions"].keys():  # iterate through current options and print them
        print("    " + str(key) + ": " + str(configDict["endpointOptions"][key]))
    print("\n")
    resp = raw_input("Would you like to update any of them? (Type the param to update it, 'y' to iterate through them all, 'n' to keep current params): ")

    if resp.lower() == "y":  # if the user wants to update, iterate through the list and query for an update
        for key in configDict["endpointOptions"].keys():
            upd = raw_input(key + ": " + str(configDict["endpointOptions"][key]) + ". Update to: ")
            if upd:
    ...
    elif resp in configDict["endpointOptions"].keys():  # user wants to change a specific option
        upd = raw_input("New value for " + str(resp) + ": ")  # ask for the new value for that option
...
def buildMapping(reader):
    ...
    ak = raw_input("Enter the column index of the Audience Key, Customer ID, or Row Number (or ENTER for none): ")
    try:
    ...
    head = raw_input("Does this file have a header row? y or n: ")
    if head.lower() == 'y':
        configDict["fileFormat"]["header"] = "y"
        head_row = reader.next()   # read the header row into a variable
        mapped_head = raw_input("Do the headers correspond to AbiliTec API params (case insensitive)? y or n: ")
        if mapped_head.lower() == 'y':  # if the headers correspond to AbiliTec API touch point params, iterate over them and map them
            for col in head_row:
                if col.lower() in [param.lower() for param in param_list.keys()]:
...
def autoMap(paramDict):
    ...
    for param in paramDict.keys():  # iterate over the parameters in the default touch point list and ask the user what column they're in
            entry = raw_input("Column index for " + param + ": " + paramDict[param] + "? ")
            try:
        ...
        try:
            for row in source_file:  # iterate through the file and add rows to be processed to a batch
                try:
                    ...
                    payload = dict(row_payload.items() + configDict["endpointOptions"].items())  # add the touch point params and optional params to a dictionary for this row
                    encoded = urllib.urlencode(payload)  # transform the payload dictionary into a URL encoded string to pass to the API for this row
                    ...
                    try:
                        ...
                        ak = row_num
                    payloadList.append(str(ak) + dl + dl.join(row_payload.values()))  # add ak and row touch points, and pass through values to a list of touch point input data (such that index of URL string list corresponds to index of row number list)
                ...
                for i in range(len(response_body)):  # the response is a batch document
                    try:
                        ...
                        if res['code'] == 200:  # each batch document also has its own status code
                            ...
                            people = [document] if 'person' in document.keys() else document['group']['people']  # create a list of all person documents
                            for doc in people:
                                result = ""
...
def checkToken(token=accessToken):
    ...
    if token["exp_time"] > (time.time() + 180):  # check if token exists with at least 180 secs before expiration
        ...
        client_secret = raw_input("Enter client_secret for client ID " + client_id + ": ")
        getToken(client_id, client_secret)  # update the global access token
        checkToken(accessToken)
